 ...
 */
class OpeningWeatherFragment : Fragment() {

    private lateinit var buttonSeeWeather: AppCompatButton
    ...
    override fun onCreateView(
        ...
        return inflater.inflate(R.layout.layout_draft_weather_open, container, false)
    }

    override fun onViewCreated(view: View, savedInstanceState: Bundle?) {
        super.onViewCreated(view, savedInstanceState)

        buttonSeeWeather = view.findViewById(R.id.button_seeWeather)
...
//                    requestBackgroundLocationPermission()
        ...
        buttonSeeWeather.setOnClickListener {
...
//                requestBackgroundLocationPermission()
    ...
    private fun openWeatherFragment() {
        (activity as MainActivity).openFragment(WeatherFragment())
    }
    ...
    private fun sendNavigationEvents() {
        requireActivity().supportFragmentManager.setFragmentResult( // Пишем requireActivity() вместо activity, т.к. activity м.б. null и нужна будет дополнительная проверка
            ...
            // В bundle указываем ключ-значение данных, которые хотим передать
            bundleOf(MainActivity.NAVIGATION_EVENT_DATA_KEY to "OpeningWeatherFragment created")
        )
    }
